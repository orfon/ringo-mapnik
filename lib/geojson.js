...
/**
 * Fast GeoJson with basic analyze tools and handlebars popups.
 */
...
GeoJson.prototype = {
    /**
     * @returns {Number} number of features in this geojson
     */
    size: function() {
    ...
    /**
     * Get all unique values for the given fieldName
    ...
    /**
     * Returns the min and max value for the field if interpreted as float.
     * @param {String} fieldName
     * @returns {Array} minimum and maximum values [min, max]
     */
    ...
    /** @ignore **/
    joinWithArray: function(array, joinField, localJoinField) {
        var features = new JsonArray();
        array.forEach(function(obj, idx){
            var id = obj[joinField];
            var feature = this.mergeClonedFeature(id, localJoinField, obj);
            if (feature != null) {
    ...
    /** @ignore **/
    joinWithCsv: function(csv, csvJoinField, localJoinField) {
    ...
    /**
     * Returns the name and example values of all fields.
     * @returns {Array} info objects {name, examples}
     */
    ...
    /**
     * Calculate the bounding box or extent of the geojson geometry.
